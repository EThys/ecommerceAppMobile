...
class _MostPupularCategoryState extends State<MostPupularCategory> {
  ...
  void _onTapItem(int index) {
    setState(() {
      ...
      final selectedCategoryId = datas[index].id;
      filteredProducts = homePopularProducts
          .where((product) => product.categoryId == selectedCategoryId)
          .toList();
    });
    print("${filteredProducts.toList()}");
  }
...
class MostPopularTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        const Text('Le plus populaire', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20, color: Color(0xFF212121))),
        TextButton(
